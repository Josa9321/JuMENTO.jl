...
"""
    add_default!(options, key, value)

Utility that sets `options[key] = value` only when the key is missing.
"""
function add_default!(options, key, value)
    if !(key in keys(options))
        options[key] = value
    end
end
