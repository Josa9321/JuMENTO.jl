function test_all_metrics()
    ...
    @test isapprox(sp1, 0.0; atol=1e-6) #"Case 1 - Spacing metric failed"
    @test isapprox(gd1, 0.0; atol=1e-6) #"Case 1 - General distance failed"
    @test isapprox(dm1, 0.0; atol=1e-6) #"Case 1 - Diversity metric failed"
    @test isapprox(me1, 0.0; atol=1e-6) #"Case 1 - Mean error failed"
    @test isapprox(ve1, 0.0; atol=1e-6) #"Case 1 - Variance error failed"
    @test isapprox(mpe1, 0.0; atol=1e-6) #"Case 1 - MPE failed"
    @test isapprox(er1, 0.0; atol=1e-6) #"Case 1 - Error ratio failed"
    @test hv1 > 0.0 #"Case 1 - Hypervolume failed"
    ...
    @test sp2 == 0.0 #"Case 2 - Spacing metric failed"
    @test gd2 > 0.0 #"Case 2 - General distance failed"
    @test dm2 > 0.0 #"Case 2 - Diversity metric failed"
    @test me2 > 0.0 #"Case 2 - Mean error failed"
    @test ve2 > 0.0 #"Case 2 - Variance error failed"
    @test mpe2 > 0.0 #"Case 2 - MPE failed"
    @test isapprox(er2, 1.0; atol=1e-6) #"Case 2 - Error ratio failed"
    @test hv2 > 0.0 #"Case 2 - Hypervolume failed"
